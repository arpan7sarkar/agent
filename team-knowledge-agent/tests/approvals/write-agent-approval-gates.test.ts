...
import type { ApprovalRequestRecord } from "../../src/approvals/approval-store.js";
...
describe("Write Agent Approval Gates", () => {
  ...
  it("blocks execution when approval is rejected", async () => {
    getApprovalRequestByIdMock.mockResolvedValue(
      buildApprovalRecord({ status: "rejected" }),
    );
  ...
  it("blocks execution when approval timed out", async () => {
    ...
    getApprovalRequestByIdMock.mockResolvedValue(
      buildApprovalRecord({ status: "approved", resolvedAt: expiredAt }),
    );
  ...
  it("executes writes only after valid approval", async () => {
    ...
    getApprovalRequestByIdMock.mockResolvedValue(
      buildApprovalRecord({ status: "approved", resolvedAt: new Date().toISOString() }),
    );
