...
describe("Session Store Isolation", () => {
  ...
  beforeEach(() => {
    ...
    withTransactionMock.mockImplementation(
      async (work: (client: { query: typeof queryMock }) => Promise<unknown>) => {
    ...
    queryMock.mockImplementation(async (sql: string, values: unknown[] = []) => {
      ...
      if (normalized.includes("update sessions")) {
        ...
        const nextState = JSON.parse(String(values[3])) as Record<string, unknown>;
        const key = `${userId}:${conversationId}`;
  ...
  it("blocks loading the same conversation from a different user", async () => {
    ...
    await expect(loadOrCreateSession(userB, conversation)).rejects.toThrow(
      "Conversation does not belong to the requested user.",
    );
  });
  ...
  it("blocks state writes when user/session scope mismatches", async () => {
    ...
    await expect(
      saveSessionState(userB, conversation, sessionId, { lastRoute: "qa" }),
    ).rejects.toThrow("Session update blocked: user/session scope mismatch.");
  });
});
